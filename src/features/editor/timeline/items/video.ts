...
class Video extends Trimmable {
	...
	public initDimensions() {
		this.thumbnailWidth = Math.max(32, Math.round(this.thumbnailHeight * this.aspectRatio));
	...
	public renderToOffscreen(force?: boolean) {
		...
		// 1) Draw background filmstrip in grayscale for the full video segment (ignore trim offset)
		{
			let timeInFilmstripeBg = startTime;
			ctx.save();
			ctx.filter = "grayscale(100%)";
			for (let i = 0; i < thumbnailsCount; i++) {
				let img = this.thumbnailCache.getThumbnail(
					Math.ceil(timeInFilmstripeBg / 1000),
				);
				if (!img) {
					img = this.thumbnailCache.getThumbnail("fallback");
				}
				if (img?.complete) {
					const xPositionBg = i * thumbnailWidth + offset; // no trimFromSize
					ctx.drawImage(img, xPositionBg, 0, thumbnailWidth, thumbnailHeight);
					timeInFilmstripeBg += timePerThumbnail;
				}
			}
			ctx.restore();
		}

		// 2) Draw colored filmstrip on top, accounting for trim offset (current behavior)
		if (this.isSelected) {
			for (let i = 0; i < thumbnailsCount; i++) {
				let img = this.thumbnailCache.getThumbnail(
