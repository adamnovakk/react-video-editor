...
const Header = () => {
	...
	const { activeGroup } = useSelectionStore();

	const currentFrame = useCurrentPlayerFrame(playerRef);

	// Yellow play state
	const [yellowMode, setYellowMode] = useState(false);
	const [yellowIndex, setYellowIndex] = useState<number | null>(null);
	...
	const selectionEntries = useMemo(() => {
		if (!activeGroup) return [] as Array<{ name: string; start: number; end: number }>;
	...
	// Yellow play effect: monitor frame and jump between segments
	useEffect(() => {
		if (!yellowMode || yellowIndex == null || !selectionEntries.length) return;
		const seg = selectionEntries[yellowIndex];
		if (!seg) return;
		const endFrame = Math.max(0, Math.round(seg.end * fps));
		if (currentFrame >= endFrame) {
			// pause and schedule jump
			dispatch(PLAYER_PAUSE);
			if (pauseTimeoutRef.current) window.clearTimeout(pauseTimeoutRef.current);
			pauseTimeoutRef.current = window.setTimeout(() => {
				const nextIndex = yellowIndex + 1;
				if (nextIndex < selectionEntries.length) {
					const nextSeg = selectionEntries[nextIndex];
					playerRef?.current?.seekTo(Math.max(0, Math.round(nextSeg.start * fps)));
					setYellowIndex(nextIndex);
					dispatch(PLAYER_PLAY);
				} else {
					setYellowMode(false);
					setYellowIndex(null);
	...
	}, [yellowMode, yellowIndex, currentFrame, fps, selectionEntries, playerRef]);

	const handleYellowPlay = () => {
		if (!selectionEntries.length) return;
		const currentSec = currentFrame / fps;
		let startIndex = selectionEntries.findIndex(
			(s) => currentSec >= s.start && currentSec < s.end,
		);
		if (startIndex === -1) {
			startIndex = selectionEntries.findIndex((s) => s.start >= currentSec);
		}
		if (startIndex === -1) return; // nothing to play

		const seg = selectionEntries[startIndex];
		playerRef?.current?.seekTo(Math.max(0, Math.round(seg.start * fps)));
		setYellowIndex(startIndex);
		setYellowMode(true);
		dispatch(PLAYER_PLAY);
	};
	...
	const handleGoBack = () => {
		...
		for (let i = 0; i < selectionEntries.length; i++) {
			if (selectionEntries[i].start < currentSec) prevIndex = i;
			else break;
		}
		const targetSec = prevIndex >= 0 ? selectionEntries[prevIndex].start : 0;
		playerRef?.current?.seekTo(Math.max(0, Math.round(targetSec * fps)));
		dispatch(PLAYER_PAUSE);
	};

	// Keyboard shortcuts: Space = toggle play, double-Space = yellow play, Cmd/Ctrl+Z = go back
	useEffect(() => {
					...
					<div className="flex items-center justify-center">
						<div>
							...
							<Button
								...
								size={"icon"}
								className="ml-2 bg-yellow-400 hover:bg-yellow-500 text-black"
