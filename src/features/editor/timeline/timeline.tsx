...
const Timeline = ({ stateManager }: { stateManager: StateManager }) => {
	...
	useEffect(() => {
		...
		const containerHeight = timelineContainerEl.clientHeight - 50; // increase available height
	...
	useEffect(() => {
		if (!timeline || !horizontalScrollbarVpRef.current || !canvasElRef.current) return;
		// Center the playhead after a zoom change
		const canvasClientWidth = canvasElRef.current.clientWidth;
		const playheadUnits = timeMsToUnits((currentFrame / fps) * 1000, scale.zoom);
		const targetScrollLeft = Math.max(0, playheadUnits - canvasClientWidth / 2 + TIMELINE_OFFSET_CANVAS_LEFT);
